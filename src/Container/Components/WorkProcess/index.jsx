...
function WorkProcess() {
    ...
    const fetchData = async () => {
        const response = await axios.get('http://127.0.0.1:8000/backend/workprocess/');
                            ...
                            <div key={index} className={`workprocess-box ${activeIndex === index ? 'active' : ''}`}>
                                <div className="wk-box" onClick={() => handleClick(index)}>
                                    ...
                                    <div className="workprocess-cicons-actiive">
                                        {activeIndex === index ? (
                                            <img src={`http://127.0.0.1:8000${process.inactiveIcon}`} alt="Inactive Icon" />
                                        ) : (
                                            <img src={`http://127.0.0.1:8000${process.activeIcon}`} alt="Active Icon" />
                                        )}
                                    </div>
